...
class WFBDMModel(SRModel):
    def __init__(self, opt):
        ...
        self.current_best_dataset = None
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('ssim_loss'):
            self.ssim_loss = build_loss(train_opt['ssim_loss']).to(self.device)
        else:
            self.ssim_loss = None

        if train_opt.get('mse_loss'):
            self.mse_loss = build_loss(train_opt['mse_loss']).to(self.device)
        else:
            self.mse_loss = None
    ...
    def feed_data(self, data):
        ...
        if 'pre' in data and data['pre'] is not None:
            self.pre = data['pre'].to(self.device)
        else:
            self.pre = self.lq
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.x_, self.noisell, self.LLrecon, self.noisehigh, self.highrecon, self.AA, self.HH = self.net_g(self.lq,
                                                                                                           self.gt)
        dwt, idwt = DWT(), IWT()
        ...
        if self.mse_loss:
            l_mse = self.mse_loss(self.x_, self.gt)
            l_total += l_mse
            loss_dict['l_mse'] = l_mse
        if hasattr(self, 'ssim_loss'):
            l_ssim = self.ssim_loss(self.x_, self.gt)
            l_total += l_ssim
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if with_metrics:
                for name, opt_ in self.opt['val']['metrics'].items():
                    ...
                    if 'gt' not in visuals or visuals.get('gt') is None:
                        if name.lower() in ['uiqm', 'uciqe', 'niqe']:
        ...
        if with_metrics:
            ...
            self.is_best_model = False
            self.current_best_dataset = dataset_name
            logger = get_root_logger()

            best_metrics_info = []
            for metric in self.metric_results.keys():
                current_val = self.metric_results[metric]
                best_val = self.best_metric_results[dataset_name][metric]['val']
                better_type = self.best_metric_results[dataset_name][metric]['better']

                is_current_best = False
                if better_type == 'higher':
                    is_current_best = current_val >= best_val
                else:
                    is_current_best = current_val <= best_val

                if is_current_best:
                    self._update_best_metric_result(dataset_name, metric, current_val, current_iter)
                    best_metrics_info.append(
                        f"{metric}: {current_val:.4f} (best: {self.best_metric_results[dataset_name][metric]['val']:.4f})")
                    self.is_best_model = True

            if self.is_best_model:
                logger.info(f"\n=== Iter {current_iter}: Best performance on {dataset_name}! ===")
                for info in best_metrics_info:
                    logger.info(f"  {info}")
            else:
                logger.info(
                    f"Iter {current_iter}: Not best performance on {dataset_name} (PSNR: {self.metric_results.get('psnr', 0):.2f}, SSIM: {self.metric_results.get('ssim', 0):.4f})")
    ...
    def get_current_visuals(self):
        ...
        if hasattr(self, 'lq') and self.lq is not None:
            out_dict['lq'] = self.lq.detach().cpu()
    ...
    def save(self, epoch, current_iter):
        if not hasattr(self, 'is_best_model') or not self.is_best_model:
            logger = get_root_logger()
            logger.info(f"Iter {current_iter}: Not a best model, skip saving")
            return

        logger = get_root_logger()
        logger.info(f"\n=== Saving best model at iter {current_iter} for dataset {self.current_best_dataset} ===")

        if hasattr(self, 'net_g'):
            self.save_network(self.net_g, 'net_g', current_iter, self.current_best_dataset, self.metric_results)

        if hasattr(self, 'net_g_ema'):
            self.save_network(self.net_g_ema, 'net_g_ema', current_iter, self.current_best_dataset, self.metric_results,
