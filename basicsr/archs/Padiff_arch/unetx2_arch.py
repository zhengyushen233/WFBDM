...
from .unet_block.cross_attention_module import CFRM
...
class SobelEdgeExtractor(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        edge_x = F.conv2d(x, self.sobel_x.repeat(c, 1, 1, 1), groups=c, padding=1)
        edge_y = F.conv2d(x, self.sobel_y.repeat(c, 1, 1, 1), groups=c, padding=1)
...
class GatedFusion(nn.Module):
    def __init__(self, dim):
        ...
        self.feature_adjust = nn.Conv2d(dim, dim, kernel_size=1)

    def forward(self, feat, high_freq):
        gate_weight = self.gate(torch.cat([feat, high_freq], dim=1))
        fused = feat + gate_weight * self.feature_adjust(high_freq)
        return fused
...
class LowFreqExtractor(nn.Module):
    def __init__(self, dim, kernel_size=3, stride=1):
        super().__init__()
        self.pool = nn.AvgPool2d(kernel_size=kernel_size, stride=stride, padding=kernel_size // 2)
        self.conv = nn.Conv2d(dim, dim, kernel_size=1)

    def forward(self, x):
        low_freq = self.pool(x)
        return self.conv(low_freq)
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.cam1 = CFRM(dim, dim)
        self.cam2 = CFRM(dim * 2 ** 1, dim * 2 ** 1)
        self.cam3 = CFRM(dim * 2 ** 2, dim * 2 ** 2)
        self.cam4 = CFRM(dim * 2 ** 3, dim * 2 ** 3)
        ...
        self.sobel_extractor = SobelEdgeExtractor()
        self.gate1 = GatedFusion(dim)
        self.gate2 = GatedFusion(dim * 2 ** 1)
        self.gate3 = GatedFusion(dim * 2 ** 2)
        self.low_freq1 = LowFreqExtractor(dim, kernel_size=5)
        self.low_freq2 = LowFreqExtractor(dim * 2 ** 1, kernel_size=5)
        self.low_freq3 = LowFreqExtractor(dim * 2 ** 2, kernel_size=5)
    ...
    def forward(self, x, t, p_t):
        ...
        high_freq1 = self.sobel_extractor(x)
        x = self.gate1(x, high_freq1)

        low_freq1 = self.low_freq1(x)
        x = x + low_freq1
        ...
        high_freq2 = self.sobel_extractor(x2)
        x2 = self.gate2(x2, high_freq2)

        low_freq2 = self.low_freq2(x2)
        x2 = x2 + low_freq2
        ...
        high_freq3 = self.sobel_extractor(x3)
        x3 = self.gate3(x3, high_freq3)

        low_freq3 = self.low_freq3(x3)
        x3 = x3 + low_freq3
...
class AdaptiveInstanceNorm2d(nn.Module):
    ...
    def forward(self, x, y):
        ...
        std_x, std_y = torch.std(x, dim=(2, 3), keepdim=True) + self.eps, torch.std(y, dim=(2, 3),
...
class UNet(nn.Module):
    def __init__(
        ...
        self.refine = ResnetBloc_eca(dim=dim * 2 ** 1, dim_out=dim * 2 ** 1, time_emb_dim=time_dim,
