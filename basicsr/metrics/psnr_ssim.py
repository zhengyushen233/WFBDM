...
@METRIC_REGISTRY.register()
def calculate_uciqe(img, img2, crop_border, input_order='HWC', **kwargs):
    img = reorder_image(img, input_order=input_order)
    ...
    var_chr = np.std(chroma)
    con_lum = np.percentile(L, 99) - np.percentile(L, 1)
...
@METRIC_REGISTRY.register()
def calculate_uiqm(img, img2, crop_border, input_order='HWC', **kwargs):
    img = reorder_image(img, input_order=input_order)
